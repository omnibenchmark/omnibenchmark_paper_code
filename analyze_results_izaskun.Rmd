...
table(fd$method, fd$method_full)
```

Choice of `k` impact in clusering perf metrics. Conda only, to speedup.

```{r, fig.width = 20, fig.height = 20}

fd_dev_k <- fd %>%
    filter(backend == 'conda') %>%
  group_by(method_full, dataset_name, metric_name) %>%
  # get the metric_value at the true_k for this group
  mutate(perf_at_true_value = metric_value[k == true_k][1],
         deviation_k = metric_value - perf_at_true_value) %>%
  ungroup()
