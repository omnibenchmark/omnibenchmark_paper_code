...
flatten_record <- function(rec) {
  ...
  for (metric_name in names(pm)) {
    ...
    true_k <- get_true_k(names(metric_vals))
...
records <- jsonlite::fromJSON('aggregated_results.json', simplifyVector = FALSE)
...
```{r}
fd_seed_summary <- fd %>%
  group_by(method, dataset_name, metric_name, k, seed) %>%
...
fd_seed_var <- fd_seed_summary %>%
  group_by(method, dataset_name, metric_name, k) %>%
...
ggplot(fd_seed_var, aes(x = metric_name, y = sd_seeds, fill = method)) +
  geom_boxplot(outlier.alpha = 0.3) +
  theme_minimal(base_size = 14) +
...
```{r}
ggplot(fd_seed_var, aes(x = metric_name, y = mean_sd_runs, fill = method)) +
  geom_boxplot(outlier.alpha = 0.3) +
  theme_minimal(base_size = 14) +
...
print(ggplot(fd_seed_summary,
             aes(x = seed, y = mean_value,
                 group = interaction(method, dataset_name, k),
                 color = method)) +
  geom_line(alpha = 0.5) +
  geom_point(size = 2) +
...
ggplot(fd_dev_true, aes(x = mean_val, y = diff_val, color = method)) +
  geom_point(alpha = 0.6) +
